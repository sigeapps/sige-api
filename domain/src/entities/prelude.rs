//! `SeaORM` Entity, @generated by sea-orm-codegen 1.1.3

pub use super::arrests::Entity as Arrests;
pub use super::assignance_time::Entity as AssignanceTime;
pub use super::band::Entity as Band;
pub use super::base::Entity as Base;
pub use super::brand::Entity as Brand;
pub use super::brigade::Entity as Brigade;
pub use super::calibre::Entity as Calibre;
pub use super::charge::Entity as Charge;
pub use super::commission::Entity as Commission;
pub use super::commission_actual_exit::Entity as CommissionActualExit;
pub use super::commission_official::Entity as CommissionOfficial;
pub use super::commission_reason::Entity as CommissionReason;
pub use super::commission_seized_transport::Entity as CommissionSeizedTransport;
pub use super::commission_transport::Entity as CommissionTransport;
pub use super::complainants::Entity as Complainants;
pub use super::confiscated_items::Entity as ConfiscatedItems;
pub use super::correspondence::Entity as Correspondence;
pub use super::correspondence_document::Entity as CorrespondenceDocument;
pub use super::country_verification::Entity as CountryVerification;
pub use super::diligencies::Entity as Diligencies;
pub use super::division::Entity as Division;
pub use super::document_type::Entity as DocumentType;
pub use super::evidence_files::Entity as EvidenceFiles;
pub use super::family_relationship::Entity as FamilyRelationship;
pub use super::flagrants::Entity as Flagrants;
pub use super::hierarchy::Entity as Hierarchy;
pub use super::inclusion_audit_log::Entity as InclusionAuditLog;
pub use super::inclusion_records::Entity as InclusionRecords;
pub use super::inclusion_records_accuseds::Entity as InclusionRecordsAccuseds;
pub use super::inclusion_records_personas::Entity as InclusionRecordsPersonas;
pub use super::init_orders::Entity as InitOrders;
pub use super::institution::Entity as Institution;
pub use super::investigations::Entity as Investigations;
pub use super::investigations_order::Entity as InvestigationsOrder;
pub use super::involved_objects::Entity as InvolvedObjects;
pub use super::issuance::Entity as Issuance;
pub use super::issuance_return::Entity as IssuanceReturn;
pub use super::issuance_weapon::Entity as IssuanceWeapon;
pub use super::judicial_presentations::Entity as JudicialPresentations;
pub use super::municipality::Entity as Municipality;
pub use super::novelty::Entity as Novelty;
pub use super::official::Entity as Official;
pub use super::organism::Entity as Organism;
pub use super::parish::Entity as Parish;
pub use super::part::Entity as Part;
pub use super::part_development::Entity as PartDevelopment;
pub use super::part_official::Entity as PartOfficial;
pub use super::part_responsability::Entity as PartResponsability;
pub use super::permission::Entity as Permission;
pub use super::persona::Entity as Persona;
pub use super::persona_children::Entity as PersonaChildren;
pub use super::persona_conyuge::Entity as PersonaConyuge;
pub use super::persona_course::Entity as PersonaCourse;
pub use super::persona_education::Entity as PersonaEducation;
pub use super::persona_health::Entity as PersonaHealth;
pub use super::persona_operational::Entity as PersonaOperational;
pub use super::persona_record::Entity as PersonaRecord;
pub use super::persona_relative::Entity as PersonaRelative;
pub use super::persona_situation::Entity as PersonaSituation;
pub use super::persona_state::Entity as PersonaState;
pub use super::persona_traits::Entity as PersonaTraits;
pub use super::persona_work_experience::Entity as PersonaWorkExperience;
pub use super::plate::Entity as Plate;
pub use super::plate_persona::Entity as PlatePersona;
pub use super::position::Entity as Position;
pub use super::profession::Entity as Profession;
pub use super::register::Entity as Register;
pub use super::role::Entity as Role;
pub use super::role_permission::Entity as RolePermission;
pub use super::seclusion::Entity as Seclusion;
pub use super::seclusion_statuses::Entity as SeclusionStatuses;
pub use super::seclusion_visit::Entity as SeclusionVisit;
pub use super::state::Entity as State;
pub use super::status_condition::Entity as StatusCondition;
pub use super::technical_expertises::Entity as TechnicalExpertises;
pub use super::temporal_seclusion::Entity as TemporalSeclusion;
pub use super::transport::Entity as Transport;
pub use super::transport_issuance::Entity as TransportIssuance;
pub use super::transport_issuance_return::Entity as TransportIssuanceReturn;
pub use super::transport_statuses::Entity as TransportStatuses;
pub use super::transport_type::Entity as TransportType;
pub use super::user::Entity as User;
pub use super::vehicle_model::Entity as VehicleModel;
pub use super::weapon::Entity as Weapon;
pub use super::weapon_brand::Entity as WeaponBrand;
pub use super::weapon_model::Entity as WeaponModel;
pub use super::weapon_type::Entity as WeaponType;
