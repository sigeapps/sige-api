use crate::{tags::TRANSPORT_TAG, Result};
use application::{
    api::ApiContext,
    dtos::{
        transport::issuance::{
    ...
    services::transport::issuance::IssuanceService,
};
use axum::{
    extract::{Path, Query},
    http::StatusCode,
...
pub struct IssuanceBody {
    issuances: Vec<TransportIssuanceSummary>,
}

#[derive(Serialize, Deserialize, ToSchema)]
pub struct SingleIssuanceBody {
    issuance: TransportIssuanceView,
}
...
#[utoipa::path(
    ...
    path = "",
    tag = TRANSPORT_TAG,
    request_body = StartTransportIssuance,
    responses(
...
pub async fn start_issuance(
    Extension(ctx): Extension<ApiContext>,
    Json(issuance): Json<StartTransportIssuance>,
) -> Result<Response> {
...
#[utoipa::path(
    ...
    path = "/finalize",
    tag = TRANSPORT_TAG,
    request_body = FinalizeTransportIssuance,
    responses(
...
pub async fn finalize_issuance(
    Extension(ctx): Extension<ApiContext>,
    Json(issuance): Json<FinalizeTransportIssuance>,
) -> Result<Response> {
